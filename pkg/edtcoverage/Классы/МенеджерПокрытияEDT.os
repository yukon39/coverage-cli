...
Процедура ОстановитьСборПокрытия(ИдентификаторПроцесса) Экспорт
	...
	Утверждения.ПроверитьРавенство(ТипЗнч(ИдентификаторПроцесса), Тип("Число"),
		"Идентификатор процесса должен быть числом");
	
	Процесс = НайтиПроцессПоИдентификатору(ИдентификаторПроцесса);
	
	Если Процесс = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Процесс с pid %1 не найден", XMLСтрока(ИдентификаторПроцесса));
	КонецЕсли;
