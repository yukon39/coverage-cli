...
Функция НайтиМодульНабораЗаписей(ПутьКИсходнымФайлам, ИдентификаторПрограммногоМодуля) Экспорт
	...
	Если КаталогОбъектаМетаданных = Неопределено Тогда
		КаталогОбъектаМетаданных = НайтиОбъектПерерасчета(ПутьКИсходнымФайлам, ИдентификаторПрограммногоМодуля);
	КонецЕсли;	
...
Функция ПерерасчетыРегистра(ФайлОбъектаМетаданных)
	...
	Пока ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		Если ЧтениеXML.ЛокальноеИмя = "recalculations" Тогда
...
Функция НайтиОбъектПерерасчета(ПутьКИсходнымФайлам, ИдентификаторПрограммногоМодуля)
	...
	Для Каждого КаталогРегистраРасчета Из КаталогиРегистровРасчета Цикл
		...
		ПерерасчетыРегистра = ПерерасчетыРегистра(ФайлОбъектаМетаданных);
		Для Каждого КлючИЗначение Из ПерерасчетыРегистра Цикл
			...
			ИмяКаталогаПерерасчета = ОбъединитьПути(КаталогРегистраРасчета.ПолноеИмя, "Recalculations", ИмяПерерасчета);
